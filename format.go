...
Details: https://juicefs.com/docs/community/quick_start_guide`,
		Flags: []cli.Flag{
			...
			// 如果使用token则设置为输入值，不传token则自动生成
			// 如果是更新format volume，则必须传token，并保持与原有token一致
			&cli.StringFlag{
				Name:  "token",
			...
			&cli.Int64Flag{
				Name:  "volume-upload-limit",
				Value: 0,
				Usage: "bandwidth limit for upload in Mbps, volume level configuration, priority is lower than mount",
			},
			&cli.Int64Flag{
				Name:  "volume-download-limit",
				Value: 0,
				Usage: "bandwidth limit for download in Mbps, volume level configuration, priority is lower than mount",
			},

			&cli.StringFlag{
				Name:  "meta-interface-rate-limit",
				Usage: "list struct 'interfaceName:burstSize' split with ','. demo is 'gets:10000,scanKeys:10000,scanValues:2000', the interfaceName support 'gets,scanKeys,scanValues'",
			},
		},
	}
}
...
func format(c *cli.Context) error {
	...
	if err == nil {
		...
		format = &meta.Format{
			...
			TokenInfo:       tokenInfo,
			TagInfo:         tagInfo,
			VolumeUpLimit:   c.Int64("volume-upload-limit") * 1e6 / 8,   // bytes per second
			VolumeDownLimit: c.Int64("volume-download-limit") * 1e6 / 8, // bytes per second
		}
		// 'gets:10000,scanKeys:10000,scanValues:2000'
		err1 := checkMetaInterfaceRateLimit(c.String("meta-interface-rate-limit"), nil)
		if err1 != nil {
			logger.Fatalf(err1.Error())
		} else {
			format.MetaInterfaceRateLimit = c.String("meta-interface-rate-limit")
		}
